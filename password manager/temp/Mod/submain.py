...
class Login(QtGui.QMainWindow):
    ...
    def AccountList(self):
        self.QDialog = passwordList(self, self.id)
        # self.QDialog.show()
    ...
    def AddIndex(self):
        self.QDialog = AddPassword(self, self.id)
        self.QDialog.show()
...
class AddPassword(QtGui.QDialog):
    ...
    def accepted(self):
        self.conn = sqlite3.connect("Data.db")
        self.c = self.conn.cursor()
        self.c.execute("INSERT INTO PASSES VALUES(?, ?, ?, ?)", [(self.id), (self.Company.text()), (self.Username.text()), (self.Password.text())])
...
class passwordList(QtGui.QDialog):
    def __init__(self, parent = None, id = 0):
        ...
        for i in range(self.counter):
            ...
            self.table.setItem(i, 2, QtGui.QTableWidgetItem(extData[i][3]))
        self.table.show()
